  ...
  return (
    <div
      className={`
        ...
        flex items-center max-w-[95vw] sm:max-w-[90vw]
        bg-[#2a2a2a]/85 backdrop-blur-md
        rounded-full border border-white/10 shadow-2xl
        transition-all duration-300 ease-in-out
        gap-1
        ${isEditorActive ? 'px-2 py-2 bottom-1 sm:bottom-1' : 'px-2 py-2 bottom-2 sm:bottom-4'}
      `}
    >
      {/* home button */}
      <button
      ...
      {openTabs.length > 0 && (
        <div className={`shrink-0 w-[1px] bg-white/15 mx-0.5 ${isEditorActive ? 'h-4' : 'h-5'}`} />
      )}
